...
public class BioSecureKvsPlugin: NSObject, FlutterPlugin {
  ...
  private func get(key: String) throws -> FlutterStandardTypedData? {
    ...
    case .secSuccess:
      if let item = item as? Data {
        return FlutterStandardTypedData(bytes: item)
      } else {
        fatalError()
      }
  ...
  private func set(key: String, data: FlutterStandardTypedData) throws {
